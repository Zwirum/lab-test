      ...
      this.interval = setInterval(function () {
        ...
        for (var row = 0; row < _this.rows; row++) {
          for (var col = 0; col < _this.cols; col++) {
            var count = _classPrivateMethodGet(_this, _getLifeNeighbourhoods, _getLifeNeighbourhoods2).call(_this, row, col),
              isLife = _this.place[row][col] === 1;
            if (isLife) {
              lifeCount++;
            }
...
function _renderLife2() {
  $('#life').append("".concat(this.step, "<hr>"));
  for (var row = 0; row < this.rows; row++) {
    for (var col = 0; col < this.cols; col++) {
      var isLife = this.place[row][col] === 1;
      $('#life').append("<span>[".concat(isLife ? 'X' : 'O', "]</span>"));
    }
    $('#life').append('<br>');
  }
}
